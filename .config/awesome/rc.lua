...
function display_brightness_notification(new_brightness)
    awful.spawn.easy_async("light -G", function(stdout, stderr, reason, exit_code)
        local title = "Brightness changed"
        local percentage = string.gsub(stdout, "\n", "") .. "%"
        local timeout = 2
        if brightness_notification == nil then
            gears.debug.print_warning("Created new icon")
            ...
            brightness_notification:replace_text(title, percentage)
            brightness_notification:reset_timeout(timeout)
        end
      end)
...
function monitor_brightness_down()
    awful.spawn("light -U 10")
    display_brightness_notification();
end

function monitor_brightness_up()
...
globalkeys = gears.table.join(
    awful.key({ }, "XF86MonBrightnessDown", monitor_brightness_down),
    awful.key({ }, "XF86MonBrightnessUp", monitor_brightness_up),
    ...
    awful.key({ modkey }, "b",
        function ()
            ...
            { description = "toggle statusbar" , group = "Custom" }),
    ...
    awful.key({ modkey }, "d", function() show_dmenu() end,
              {description = "show the menubar", group = "launcher"})
)

function show_dmenu()
    awful.spawn.with_shell("~/.scripts/run_rofi.sh")
end
...
-- 	    bg_normal = beautiful.border_normal,
-- 	    bg_focus  = beautiful.boder_focus,
--     })
end)
